...
def get_stereo_opt(threads=28, kernel=(35,35), nlevels=5, spr=1, timeout=360, erode=0, align='None',
    ...
    stereo_opt = []
    #This is irrelevant
    stereo_opt.extend(['-t', 'nadirpinhole'])
    ...
    stereo_opt.extend(['--ip-per-tile','2000'])
    stereo_opt.extend(['--ip-detect-method','1'])
    stereo_opt.extend(['--ip-num-ransac-iterations', '1000'])
    #stereo_opt.extend(['--ip-inlier-factor', '2.0'])
    stereo_opt.extend(['--disable-tri-ip-filter'])
    ...
    if spr > 3:
        #TODO:
			# generalise selection of SGM vs MGM
            # kernel size of sub-pixel mode ? (Selected from 4 to 12)
            # For now, with default SGM/MGM implementation, user should pass 12 to invoke SGM without any filtering
            # intelligent threading and number of process per thread division
        #Use SGM
...
def main():
    ...
    if spr > 3:
        ...
        # sgm/mgm will likely require parallel stereo for larger tiles
        # maximum can go upto 9000*9000 px tile size, although that might make gdal crash (see ASP doc)
        # for now fix max tile size as 6400 * 6400 px, and if ds size is larger, use parallel stereo
        ds1_clip = iolib.fn_getds(ds1_clip_fn)
        ds_x = ds1_clip.RasterXSize  # ds1_clip and ds2_clip will have same extent due to warplib
        ds_y = ds1_clip.RasterYSize
        ds1_clip = None
        if (ds_x > 6400) | (ds_y > 6400):
            print(f"Clipped image dimension is {ds_x} x {ds_y} px, will use parallel_stereo")
            corr_tile_size = 1024 # suggested by Scott M.
            parallel_stereo = True
        else:
            # use sgm with normal stereo with tile size of 6400 * 6400 px
            print(f"Clipped image dimension is {ds_x} x {ds_y} px, will use normal stereo")
            corr_tile_size = 6400
            parallel_stereo = False
    else:
        # this is non-sgm
        corr_tile_size = 1024 # not-relevant
        parallel_stereo = False
            ...
            if os.path.exists(vx_fn) and os.path.exists(vy_fn):
                ...
                if t_factor is not None:
                    ...
                    #Otherwise, generate a D_sub map from low-res velocity
                        ...
                        gen_d_sub(d_sub_fn, dx, dy,pad_perc=.7)
